  ...
  Widget build(BuildContext context) {
    return BlocListener<AuthCubit, AuthState>(
      listener: (context, state) {
        ...
        } else if (state is AuthGoToQuiz) {
          context.pushNamedAndRemoveAll(Routes.quiz);
        } else if (state is AuthPlacementError) {
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: BlocBuilder<AuthCubit, AuthState>(
                      buildWhen: (_, s) =>
                          s is AuthCheckingPlacement ||
                          s is AuthInitial ||
                          s is AuthPlacementError,
                      builder: (context, state) {
                        final isLoading = state is AuthCheckingPlacement;
                        return CustomElevatedButton(
                          onTap: isLoading
                              ? null
                              : () => context
                                  .read<AuthCubit>()
                                  .checkPlacementAndNavigate(),
                          text: isLoading ? '...' : AuthStrings.startGame,
